...
Vite builds directly into `docs/` with base path `/mastering-studio-wasm/`. The `docs/` directory is committed and explicitly not gitignored. `404.html` is copied from `index.html` for SPA fallback behavior.

## Consequences

- A push to `main` updates the live static app.
- Built assets use hashed filenames for cache busting.
- The repository contains generated Pages artifacts.
